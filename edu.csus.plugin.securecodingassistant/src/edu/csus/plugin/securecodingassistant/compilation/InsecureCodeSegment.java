...
class InsecureCodeSegment {
	...
	public InsecureCodeSegment(ASTNode node, IRule rule, ReconcileContext context) {
		...
		// Set the disclaimer for CERT if this is a CERT rule
		String message_note = "";
		if(rule instanceof SecureCodingRule) {
			SecureCodingRule srule = (SecureCodingRule)rule;
			message_note = String.format("NOTE: The text and/or code below is from the "
							+ "CERT website %s%n%n", srule.getRuleURL());
		}
		...
		try {
			...
			m_marker.setAttribute(IMarker.MESSAGE,
					String.format("Rule violated: %s%nSeverity: %s%n%n"
							+ "%s" // Message note if applicable
							+ "Rule description: %s%n%n"
							+ "Rule Solution: %s", rule.getRuleName(), severity, message_note,
